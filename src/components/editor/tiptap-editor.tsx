...
import ImageOptionsDialog, {
  type ImageAlign,
  type ImageOptionsValue,
} from "./image-options-dialog";
import {
  BoldIcon,
  ItalicIcon,
  UnderlineIcon,
...
export default function TiptapEditor({ content, onChange, placeholder = "내용을 입력하세요...", uploadEndpoint }: TiptapEditorProps) {
  ...
  const [imageDialogValue, setImageDialogValue] = useState<ImageOptionsValue | null>(null);
  ...
  const openInsertDialog = (src: string) => {
    setImageDialogMode("insert");
    setImageDialogValue({ src, align: "center", href: null });
  ...
  const openEditDialog = () => {
    ...
    setImageDialogValue({
      src: attrs.src,
      align: (attrs.align as ImageAlign) ?? "left",
      href: attrs.href ?? null,
    });
    setImageDialogOpen(true);
  };

  const handleImageDialogSubmit = (value: ImageOptionsValue) => {
    if (imageDialogMode === "edit" && editor.isActive("image")) {
